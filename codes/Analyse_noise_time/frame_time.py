...
def m_slice(path, path2, dir, step, ex):
    ...
    path_head = dir + '_' # str型の名前をpath_headに代入してるだけ
    ...
    for i in range(Fs - 1):
        ...
        p = '/Users/ryuma/Desktop/コンテスト関連/ITU/codes/Analyse_noise_time/' # ファイル出力するときのパス
        ...
        if flag == True:
            if True in check: # ここら辺は出力するフレームの数字が綺麗になるように処理しているだけ
                ...
                print(f_time)
                w_pixel = np.append(w_pixel, count_diff(p, path_out, re_path_out)) # ノイズピクセルを時間軸で数える
                print(w_pixel)
            else:
                pass
        else:
            pass
    graph_pixel(f_time, w_pixel) # グラフ絵画
    return
...
def count_diff(path, o_path, r_path): # ノイズピクセルを時間軸で数える
...
def graph_pixel(f_time, w_pixel):
    fig, ax = plt.subplots()
    c1 = "red"
    l1 = "number of noise pixels"
    ...
    ax.plot(f_time, w_pixel, color=c1, label=l1)

    ax.legend(loc=0)
    fig.tight_layout()
    plt.savefig('Noise_time_graph.png')
    plt.show()

m_slice('original.mp4', 'received.mp4', 'original', 100, '.png') # プログラムの実行
